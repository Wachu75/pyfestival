...
class hangman_frame(tk.Frame):
    ...
    def check_answer(self, key=None):
        ...
        if win == False:
            messagebox.showerror(title="Przykro mi...", message=f"Przegrałaś :-(\n Prawidłowa odpowiedź to: {self.game.word}")
            self.setup_game()
        elif win == True:
...
class Hangman:
    ...
    def check_letter(self, letter):
        if letter in self.word:
            ...
            if '#' not in self.tip:
                self.won = True
            return (self.won, True, self.step, self.tip, self.wrong_letters)  # (stan gry, czy trafiona litera, ilość błędów, odkryte litery)
        else:
            self.step += 1
            if self.step >= 7:
                self.won = False
